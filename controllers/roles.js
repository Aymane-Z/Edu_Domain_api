...
import exportListPage from '../exports/roles_list.js';
import exportViewPage from '../exports/roles_view.js';
...
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(req.query.export){
			query.attributes = DB.Roles.exportListFields();
...
router.get('/view/:recid', async (req, res) => {
	try{
		...
		query.where = where;
		if(req.query.export){
			query.attributes = DB.Roles.exportViewFields();
			let records = await DB.Roles.findAll(query);
