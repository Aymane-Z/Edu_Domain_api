...
import exportListPage from '../exports/utilisateurs_list.js';
import exportViewPage from '../exports/utilisateurs_view.js';
...
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(req.query.export){
			query.attributes = DB.Utilisateurs.exportListFields();
		...
		if(uploadedPaths.length){
			uploadedPaths.forEach(function (fpath){
				...
				csv.fromPath(fpath, {headers: true, ignoreEmpty: true}).on("data", function(data){
					...
					try{
						let affectedRows = await DB.Utilisateurs.bulkCreate(records);
...
router.get('/view/:recid', async (req, res) => {
	try{
		...
		query.where = where;
		if(req.query.export){
			query.attributes = DB.Utilisateurs.exportViewFields();
			let records = await DB.Utilisateurs.findAll(query);
...
, async (req, res) => {
	try{
		...
		record = await DB.Utilisateurs.findOne(query);//for audit trail
		const newValues = JSON.stringify(record); 
