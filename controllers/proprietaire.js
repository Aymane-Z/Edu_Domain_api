...
import exportListPage from '../exports/proprietaire_list.js';
import exportViewPage from '../exports/proprietaire_view.js';
...
/**
 * Route to list proprietaire records
 * @GET /proprietaire/index/{fieldname}/{fieldvalue}
 */
router.get(['/', '/index/:fieldname?/:fieldvalue?'], async (req, res) => {  
	try{
		...
		if(search){
			let searchFields = DB.Proprietaire.searchFields();
		...
		if(req.query.export){
			query.attributes = DB.Proprietaire.exportListFields();
			let records = await DB.Proprietaire.findAll(query);
			return exportListPage(records, req, res)
		}
		query.attributes = DB.Proprietaire.listFields();
		let page = parseInt(req.query.page) || 1;
		let limit = parseInt(req.query.limit) || 10;
		let result = await DB.Proprietaire.paginate(query, page, limit);
		...
		if(uploadedPaths.length){
			uploadedPaths.forEach(function (fpath){
				...
				csv.fromPath(fpath, {headers: true, ignoreEmpty: true}).on("data", function(data){
					...
					try{
						let affectedRows = await DB.Proprietaire.bulkCreate(records);
...
/**
 * Route to view Proprietaire record
 * @GET /proprietaire/view/{recid}
 */
router.get('/view/:recid', async (req, res) => {
	try{
		...
		query.where = where;
		if(req.query.export){
			query.attributes = DB.Proprietaire.exportViewFields();
			let records = await DB.Proprietaire.findAll(query);
			return exportViewPage(records, req, res)
		}
		query.attributes = DB.Proprietaire.viewFields();
		let record = await DB.Proprietaire.findOne(query);
		if(!record){
			return res.notFound();
		}
...
/**
 * Route to insert Proprietaire record
 * @POST /proprietaire/add
 */
router.post('/add/', 
	[
...
, async function (req, res) {
	try{
		...
		//save Proprietaire record
		let record = await DB.Proprietaire.create(modeldata);
...
/**
 * Route to get  Proprietaire record for edit
 * @GET /proprietaire/edit/{recid}
 */
router.get('/edit/:recid', async (req, res) => {
	try{
		...
		query.raw = true;
		query.where = where;
		query.attributes = DB.Proprietaire.editFields();
		let record = await DB.Proprietaire.findOne(query);
		if(!record){
			return res.notFound();
		}
...
/**
 * Route to update  Proprietaire record
 * @POST /proprietaire/edit/{recid}
 */
...
, async (req, res) => {
	try{
		...
		await DB.Proprietaire.update(modeldata, {where: where});
		record = await DB.Proprietaire.findOne(query);//for audit trail
		const newValues = JSON.stringify(record); 
...
/**
 * Route to delete Proprietaire record by table primary key
 * Multi delete supported by recid separated by comma(,)
 * @GET /proprietaire/delete/{recid}
 */
router.get('/delete/:recid', async (req, res) => {
	try{
		...
		await DB.Proprietaire.destroy(query);
		return res.ok(recid);
	}
	catch(err){
		return res.serverError(err);
